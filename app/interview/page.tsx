...
type Report = {
  ...
  perQuestionFeedback: Array<{ question: string; answer: string; whatWentWell: string; whatCouldBeBetter: string; betterApproach: string }>;
};
...
function countFillers(answer: string) {
  return Object.fromEntries(fillerWords.map((word) => [word, (answer.match(new RegExp(`\\b${word}\\b`, "gi")) ?? []).length]).filter(([, count]) => count > 0));
}
...
export default function InterviewPage() {
  ...
  async function submitAnswer() {
    ...
    const nextConversation = [...conversation, { question, answer: candidateAnswer, answerMeta: { durationSeconds, fillerWords: countFillers(candidateAnswer), speechRecognitionUsed: speechSupported } }];
      ...
      <div className="mx-auto w-full max-w-5xl">
        {phase === "setup" ? (
          <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-10">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">We&apos;ll use your resume and role to ask seven tailored questions. Your camera and microphone are used in the browser and stop when the session ends.</p>
